...
userRouter.post("/register", async (req, res) => {
    let { username, email, password, bills } = req.body;
            ...
            bcrypt.hash(password, 5, async (err, hash) => {
                if (hash) {
                    const user = new UserModel({
                        username, email, password: hash, bills: []
                    })
                    await user.save()
...
userRouter.patch("/:id/bill", auth, async (req, res) => {
    const { id } = req.params;
    try {
        ...
        res.status(200).send({ "message": "bills is updated", "userDetails": newUser })
    } catch (error) {
